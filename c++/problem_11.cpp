...
constexpr long grid_side = 20;
constexpr unsigned char grid[grid_side][grid_side] {
...
long solve() {
	long largest = 0;
	for (long i = 0; i < grid_side - 3; ++i) {
		for (long j = 0; j < grid_side - 3; ++j) {
			long prod;
			...
			prod = grid[i+3][j] * grid[i+2][j+1] * grid[i+1][j+2]
				* grid[i][j+3];
			if (prod > largest) {
				largest = prod;
			}
